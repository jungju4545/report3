...
public class Crawlering {
	...
	public String parsing(Company c, Model model) {
		...
		try {
			   cr.delete();
			   // 마지막 페이지에 있는 고용건수가 40개 미만인걸 이용해보자.
//			for (int i = 0; i < 110; i++) {
System.out.println("=======페이지 ==================================");
				Document doc
				= Jsoup.connect("http://www.jobkorea.co.kr/recruit/joblist?menucode=duty&duty=1000100#anchorGICnt_4").get();
				// connet는 주소를 가지고 온다.
